...
export async function deleteAccountRequest(token) {
  const serverResponse = await fetch(BACK_URL + "/", {
    method: "DELETE",
...
export async function updateSkaterTag(token, uID, newSkaterTag) {
  const serverResponse = await fetch(BACK_URL + "/user/skaterTag", {
    method: "PUT",
    headers: {
      ...headers,
      Authorization: token,
  ...
  if (!serverResponse.ok) {
    console.error("Erreur API SkaterTag:", responseJson);
    throw new Error(responseJson.reason || "Erreur inconnue");
  }
...
export async function deleteVideo(token, videoID) {
  const serverResponse = await fetch(BACK_URL + `/video/${videoID}`, {
    method: "DELETE",
    headers: { ...headers, Authorization: token },
  });
  return serverResponse.json();
